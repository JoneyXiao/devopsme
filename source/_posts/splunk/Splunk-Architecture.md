...
- Search head cluster, and a **deployer** governs the members of the search head cluster which distributes apps, files and configuration updates through a configuration bundle
...
Two cloud experiences:

- Classic
- Victoria: new customers will be on Victoria which is hosted on AWS

splunk cloud is licensed through subscription types

- Splunk cloud is subscription based
- workload-based subscription is the default, it's based on resource capacity used, not data volume ingested
- ingest-based subscription, it requires an exception and more like the splunk enterprise license, it's based on the volume of uncompressed data to index on a daily basis, it includes a fixed amount of data storage and can purchase additional storage as needed
...
- Splunk adds default fields to all events, the default fields are all required
    ...
    - `host`: the hostname of ip address of the source system
    - `sourece`: the name of the file stream or other input from which the event originated
    - `source_type`: the format of the data, for example, a cisco syslog will come in as a source type `cisco_syslog`, a csv file will come in as a source type `csv`.
...
- Hot bucket: newly indexed data, it's actively written to and searched, it's the data that's most available to a search head. After a certain amount of time, the data ages out into the warm bucket.
- Warm bucket: The warm bucket has no active writing, then the data is aged and rolled to a cold bucket.
- Cold bucket: the data is aged and rolled to a frozen bucket.
...
- Thawed bucket: when restoring from a frozen bucket, the data is thawed into a thawed bucket.
...
As the deployments data volume increases
